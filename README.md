...
>
> * `덧셈 연산자 +` : 숫자를 더한 결과 반환, 이항 연산자 +의 피연산자로 문자열 전달 --> 문자열 병합(연결)함
> * `뺄셈 연산자 -`
> * `곱셈 연산자 *`
> * `나눗셈 연산자 /`
> * `나머지 연산자(remainder operator) %` : 비율(퍼센트)과는 관련 없음, 나눈 후 정수 반환
> * `거듭제곱 연산자(exponentiation operator) **` : a\*\*b a를 b번 곱한 값이 반환됨
